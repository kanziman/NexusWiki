...
-- -----------------------------------------------------------------------------
-- 2. search_chunks 제거 — 인자 타입이 바뀌므로 replace가 아니라 drop이다
--
-- ⚠️ create or replace는 인자 타입이 달라지면 기존 함수를 대체하지 않고 **새
--    오버로드를 만듭니다**. 그러면 1536 시그니처와 1024 시그니처가 공존하고,
--    PostgREST는 인자 이름으로 후보를 고르므로 어느 쪽이 불릴지가 호출마다
--    바뀝니다. 계약 테스트(supabase/tests/0008_search_contract.sql)가 pg_proc의
--    행 수를 1로 단언하는 이유가 이것입니다.
--
-- 함께 사라지는 것: 0007:386-387이 건 EXECUTE 권한. 섹션 6이 복원합니다.
-- -----------------------------------------------------------------------------
