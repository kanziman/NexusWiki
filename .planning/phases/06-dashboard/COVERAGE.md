...
| `sign_up` (self-service) | OPT-OUT | No signup copy exists anywhere in `06-UI-SPEC.md`'s Copywriting Contract; accounts are provisioned outside the dashboard (Studio / admin), consistent with the invite flow in `06-03-PLAN.md` requiring the invitee to already hold an account |
...
| `GET /workspaces/{id}/graph` (`wiki_graph_neighborhood` RPC) | OPT-OUT | UI-06's canvas is built on direct `wiki_pages`/`wiki_links` PostgREST reads to satisfy the explicit "1000-row PostgREST cap" requirement; the neighborhood RPC's independent 200-row cap serves a different "explore from one seed node" interaction not in this phase's literal UI-SPEC scope. Rationale documented in `06-08-PLAN.md` |
