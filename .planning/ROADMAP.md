...
- [x] 02-05-PLAN.md — 공용 한국어 토크나이저(`normalize`/`bigram`/`bigram-nfkc-cf-v1`)와 결정적 슬러그(`slug_v1`, 한글 유지)
...
- [x] 02-08-PLAN.md — Railway `asia-southeast1` 실측 기반 noop 큐 오버헤드 기준선과 잠정 reap 타임아웃 문서
...
- [x] 03-03-PLAN.md — `packages/core` 도메인 enum(`DB_CHECK_ENUMS`)과 토큰 기준 청킹(`content[char_start:char_end]` 왕복 속성)
...
- [x] 03-05-PLAN.md — 파일·URL·텍스트 세 인큐 경로 · 스트리밍 크기 상한 · 요청자 JWT Storage 업로드 · 402 비용 거부
- [x] 03-06-PLAN.md — PDF·HTML·평문 추출과 품질 게이트(`needs_ocr`) · SSRF 가드가 붙은 URL 페치 · `service_role` Storage 다운로드
...
- [x] 04-09-PLAN.md — RTV-04 gap closure: pin runner identity (`git_sha`) into `compare_order_records()`/`compare_graph_records()`, reject the invalid v4 strict/relaxed pair, and regenerate a valid single-commit v5 pair
...
- [x] 06-02-PLAN.md — 워크스페이스 스위처(D-03) + 5개 표면 내비게이션
- [x] 06-03-PLAN.md — 멤버 명부 + 이메일 초대(D-04) — 마이그레이션 0014(신규 RPC 2종) + [BLOCKING] db push
- [x] 06-04-PLAN.md — 공용 apps/api 인증 fetch 래퍼(`lib/api-client.ts`) + SSE 프레임 파서(`lib/sse.ts`)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 06-05-PLAN.md — 드롭존 3탭 수집(D-06) + 실제 5단계 잡 스테퍼(D-05) + 재시도/취소
- [x] 06-06-PLAN.md — Ask UI — 인용 앵커 스트리밍 placeholder→해소(D-09), 사이드 패널(D-10), 근거 없음 카드(D-11)
- [ ] 06-07-PLAN.md — 위키 뷰어 — 읽기 전용 배너, WikiLink 내비게이션, 레드 링크 CTA, 검증/충돌 콜아웃
- [x] 06-08-PLAN.md — Cytoscape 지식 캔버스 — 렌즈 필터, PostgREST 1000행 상한 처리
