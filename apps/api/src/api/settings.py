...
class ApiSettings(BaseAppSettings):
    ...
    **명시적 허용 목록**으로 고정한다 — 그 목록에 없는 필드가 생기면 red가 된다.
    ...
    # 브라우저 CORS 허용 origin 목록 (콤마 구분 문자열 — Railway 대시보드에서 값 하나로
    # 넣기 쉽도록 JSON 배열 대신 이 형태를 고른다). 자격증명이 아니라 운영 토글이다.
    # ⚠️ Phase 6(대시보드) 전에는 apps/api를 브라우저에서 직접 호출한 적이 없어 이
    # 미들웨어 자체가 존재하지 않았다 — 실측: 로컬 대시보드에서 소스 등록 시도가
    # 전부 "소스 등록에 실패했습니다"로 실패, OPTIONS 프리플라이트가 405로 응답.
    # 기본값은 로컬 개발용이며, 배포 시 실제 프론트엔드 도메인으로 덮어써야 한다.
