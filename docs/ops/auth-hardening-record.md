...
2026-08-05 15:44 KST

### 방법

- publishable key는 `apikey` 헤더에만 넣어 클라우드 Auth HTTP 엔드포인트를 호출했다.
- 12자 이상 가입으로 만든 테스트 계정은 성공·실패 경로 모두에서 Admin API로 정리하도록 `trap`을 사용했다.
- 응답 본문, URL 전체, 키 값은 출력하거나 기록하지 않았다.

### 결과

- 6자 비밀번호 가입: HTTP `422`, 오류 코드 `weak_password`, 거부 확인.
- 12자 이상 비밀번호 가입: HTTP `200`, `email_confirmed_at = null`, 확인 대기 상태 확인.
- 미확인 이메일 로그인: HTTP `400`, 오류 코드 `email_not_confirmed`, 거부 확인.
- 테스트 이메일: `n***@gmail.com`
- 삭제한 테스트 계정 수: `1`
- 정리 후 클라우드 `auth.users` 행 수: `0`
- 종합 판정: `auth_hardening: ok`
