...
### 사용자 생성 묶음은 필요하다 — 단 평면으로 [설계 예정]

⚠️ **카테고리는 필터이지 그릇이 아니다.** 둘을 바꿔치기하지 않는다.

| | 컬렉션 | 카테고리 |
| --- | --- | --- |
| 만드는 주체 | **사용자** (`[+]` 로 생성·명명) | 생성 불가, 4종 고정 |
| 배정 주체 | 사용자가 문서를 넣는다 | 컴파일러가 페이지마다 배정 |
| 성격 | 그릇(container) | 필터(lens) |
...
스코프 셀렉터는 `현재 문서 주변(wiki_links 이웃)` · `카테고리` · `워크스페이스 전체` 로 구성한다. 컬렉션이 들어오면 `컬렉션` 단계가 여기에 추가된다.
...
### [금지] 공개 표면에서 내부 라우트로 링크하지 않음

* **원칙**: 발행본은 내부 위키 본문의 스냅샷이라 `[[문서명]]` 표기가 그대로 들어 있다. 공개 표면은 이 표기를 **제목 평문으로 펼치고, 링크는 만들지 않는다.**
* ⚠️ 링크로 만들면 두 가지가 동시에 깨진다:
  1. 대상이 `/w/[workspace_id]/wiki/...` 내부 라우트다. `anon` 은 도달할 수 없고, **링크를 그리는 순간 워크스페이스 식별자가 마크업으로 새어 나간다.**
  2. 브래킷을 그대로 두면 외부 열람자에게 내부 마크업이 노출된다.
* 위의 `workspaces` 조인 금지가 **조회 계층**의 경계라면, 이것은 같은 경계의 **렌더 계층** 버전이다. 조회가 막혀 있어도 렌더가 내부 주소를 그리면 정보는 샌다.
* `/p/` 뿐 아니라 앞으로 생길 모든 공개 표면(공개 색인, 임베드, OG 이미지, 공개 검색)에 걸린다.
