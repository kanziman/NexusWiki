...
> **문서 상태**: 리뷰 반영판 (2026-08-17). 이전 "확정(Validated)" 판의 지원 형식·필터 축·성능 계약을 실제 구현과 대조해 정정했다. `.sql`·`.csv`·`.json` 추가는 검토 후 제외로 결론냈다(§2.2).
...
### 2.2 검토 후 제외한 형식 — 결정 기록

`.sql` · `.csv` · `.json` · `.zip` 은 **이번 마일스톤에서 지원하지 않는다.** 지원 형식은 §2.1 의 3종을 유지한다.

한 차례 추가하기로 했다가 아래 사유로 되돌린 결정이므로, 같은 제안이 다시 올라오지 않도록 근거를 남긴다.

**`.csv` · `.json` — 이 제품의 핵심 가치와 충돌한다.**
파이프라인은 `추출 → 청킹 → 임베딩 → 위키 컴파일` 이고 청커는 산문을 전제한다(`recursive-cl100k-512-64-v1`, 문장 경계 `[.!?。！？]` 기준). 표나 객체 배열에 이 규칙을 적용하면 청크 경계가 레코드 중간을 자른다. 그러면 인용 스니펫이 문장으로 읽히지 않고, **이중 Citation 의 원문 쪽이 깨진다** — 이 제품이 "또 하나의 RAG 챗봇"과 갈라지는 지점이 정확히 여기다. 임베딩도 반복되는 열 이름에 지배되어 변별력을 잃는다.

→ 지원하려면 레코드 단위 청킹이 선행되어야 한다. 그건 별도 과제이지 형식 목록에 한 줄 더하는 일이 아니다.

**`.sql` — 위 문제는 덜하지만 단독으로 넣을 이유가 약하다.**
DDL 은 주석이 섞인 준산문이라 청킹 품질 위험은 `.csv`·`.json` 보다 낮다. 다만 브라우저가 보내는 MIME 이 일정하지 않아(`application/sql` · `text/x-sql` · `text/plain` · `application/octet-stream` · 빈 문자열) MIME 허용 목록만으로는 정상 파일이 거부된다. 지원하려면 확장자 폴백 경로가 필요하다.
현재는 `.sql` 을 `text/plain` 으로 올리는 우회가 가능하므로 급하지 않다. 재검토 시 이 형식만 단독으로 다룬다.

**`.zip`** — 압축 해제·중첩·zip bomb 방어가 별도 과제다.

**구현 시 알아둘 함정** (형식을 늘리는 시점에 유효):
`extract.py` 의 `SUPPORTED_MIME_TYPES` 에만 추가하고 `_TEXT_MIME_TYPES` 를 빠뜨리면, `extract_text` 의 마지막 분기인 **HTML 파서**로 떨어져 `<` 를 태그로 오인한다. JSON·SQL 내용이 조용히 잘린다.

### 2.3 짧은 파일 거부 사유가 오해를 부른다 — [현재 형식에서도 발생]

품질 게이트가 `MIN_TOTAL_CHARS = 200` 인데 미달 시 사유가 `needs_ocr` 다. PDF 를 전제한 이름이라, 짧은 텍스트·마크다운 파일을 올린 사용자에게는 뜻이 통하지 않는다. 사유 코드를 형식에 맞게 분기하거나 UI 문구에서 재해석해 보여준다.
...
지원 MIME 이 3종이므로 탭도 3개다: `전체` · `PDF` · `텍스트/마크다운`.
탭은 `mime_type` 집합으로 정의한다 — `텍스트/마크다운`은 `text/plain`+`text/markdown` 을 묶는다. 형식이 늘면 탭 정의만 늘리고 축(`mime_type`)은 바꾸지 않는다.
...
* 드래그앤드롭 + 지원 형식 안내는 **§2.1 의 실제 값**을 쓴다: `PDF · 텍스트 · 마크다운, 최대 20MB`.
  * §2.2 에서 제외한 `.sql`·`.csv`·`.json`·`.zip` 을 예시로 보여주지 않는다.
...
| 2 | 업로드 | §2.1 의 3종 MIME 만 통과하고, 20 MiB 초과와 §2.2 제외 형식이 거부되는지 |
