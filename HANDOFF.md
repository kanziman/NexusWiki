...
- [ ] **`.claude/skills/` 하위 840개 파일이 working tree 에서 삭제된 상태**(`git status` 상 `D`, 미스테이징). **이번 세션에서 내가 지운 것이 아니다.** 이전 세션 HANDOFF 가 `.planning/` 227개 삭제로 똑같이 경고했던 패턴이다. 의도된 것인지 사용자에게 확인하고 나서 커밋하거나 복구할 것 — **임의로 커밋하지 말 것.**
...
> "HANDOFF.md 확인하고, `.claude/skills/` 840개 삭제가 의도된 것인지부터 확인한 다음 workspace-settings PRD 리뷰를 이어서 진행해줘."
